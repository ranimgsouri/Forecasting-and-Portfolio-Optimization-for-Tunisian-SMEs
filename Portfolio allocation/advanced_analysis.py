...
df = pd.read_csv("forecasted_returns_matrix.csv", index_col=0)
...
for company in df.columns:
    ...
    classification.append({
        "Company": company.replace("_xgboost_forecast", ""),
        "Mean Return": mu,
        "Volatility": sigma,
        "Zone": zone
    })
...
result_df.to_csv("advanced_risk_return_matrix.csv", index=False)
...
plt.title("Risk vs. Return Classification of Tunisian SMEs")
...
plt.ylabel("")
plt.title("Distribution of Companies by Risk-Return Classification")
plt.tight_layout()
plt.savefig("zone_distribution_pie.png")
plt.show()
